...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        assert (next_qs.ndim == 2), f"next_qs should have shape (num_critics, batch_size) but got {next_qs.shape}"
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = self.target_critic(next_obs, next_action)  # bootstrapping from the target critic
            ...
            target_values: torch.Tensor = reward + self.discount * (1 - done.long()) * next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # rsample: reparameterized sampling -> lower variance
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            assert action.shape == (self.num_actor_samples, batch_size, self.action_dim,), action.shape
            ...
            assert q_values.shape == (self.num_critic_networks, self.num_actor_samples, batch_size,), q_values.shape
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_infos.append(self.update_critic(obs=observations, action=actions, reward=rewards, next_obs=next_observations, done=dones))
        ...
        assert self.soft_target_update_rate or self.target_update_period
