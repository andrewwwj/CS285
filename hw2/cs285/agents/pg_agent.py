...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = ptu.to_numpy(self.critic(ptu.from_numpy(obs)))  # values = baseline
            assert values.squeeze().shape == q_values.shape
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        list_discounted_returns = np.repeat(sum_discounted_returns, T)

        return list_discounted_returns
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        list_discounted_reward_to_go = np.zeros(T)
        for t in range(T):
            list_discounted_reward_to_go[t] = np.sum(gammas[:T - t] * rewards[t:])

        return list_discounted_reward_to_go
